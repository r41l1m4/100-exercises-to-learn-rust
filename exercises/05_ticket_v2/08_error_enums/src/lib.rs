...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::TitleError {cause}) => panic!("{}", cause),
        Err(TicketNewError::DescriptionError {cause: _})
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            ...
            return Err(
                TicketNewError::TitleError {
                    cause: "Title cannot be empty".to_string()
                });
        }
        if title.len() > 50 {
            ...
            return Err(
                TicketNewError::TitleError {
                    cause: "Title cannot be longer than 50 bytes".to_string()
                });
        }
        if description.is_empty() {
            ...
            return Err(
                TicketNewError::DescriptionError {
                    cause: "Description cannot be empty".to_string()
                });
        }
        if description.len() > 500 {
            ...
            return Err(
                TicketNewError::DescriptionError {
                    cause: "Description cannot be longer than 500 bytes".to_string()
                });
        }
